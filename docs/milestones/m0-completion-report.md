...
三套实现的功能约定一致，但调用接口不同：Reference／Fast 接收逻辑向量，
Detailed 接收端口映射并由 Simulation 驱动。它们没有共享同一个 NOT 求值函数。
测试再以真值表字面量作为外层断言，降低同源错误导致“自己证明自己”的风险。
...
验收命令和各类证据的详细解释见 [M0 验证报告](../validation/m0.md)。
以下是 2026-08-30 的历史结果；快速连续输入缺陷与后续修正见第 12 节。
...
5. 当前基础可供 M1 复用；寄存器仍需扩展组件状态与采样／提交契约，不能直接套用组合逻辑延迟。
...
- trace 保存已提交的值变化，不包含无变化待处理项；进入更大模型前，需要设计可配置追踪，同时不能破坏审计语义。
...
## 12. 固定传输延迟修正（Fixed Transport Delay Correction）

2026-09-06 对基线 `f0fea79` 复现出快速连续输入漏调度：
delay=3、初始输入 1／输出 0，t=7 输入 0、t=8 恢复 1，旧实现最终停在输入 1／输出 1。
原 25 项测试的通过不覆盖这个场景，因此不能沿用原验收来证明连续输入正确。

修正采用 [ADR 0002](../decisions/0002-fixed-transport-delay.md)：
每次组合逻辑求值的输出都入队，仅在实际提交时去掉无变化项。
复用现有队列和因果字段，没有增加延迟模式、取消机制或外部依赖。

新增 19 项测试（其中一项含 200 组属性测试），覆盖短／等／长间隔、
同刻输入、零延迟、四态与重复输入、两级传播、确定性和重放。
本地总计 44 项测试通过、语句覆盖率 79.06%，Ruff、mypy、三模型验证和 GUI smoke 通过。
完整证据、命令及固定短脉冲波形见 [M0 验证报告](../validation/m0.md)。

原实验文件保持不变且重放通过；新增 `experiments/m0-not-short-pulse.json`。
格式仍是 v1，但其他旧记录可能受序号、时间和修正后波形影响而出现摘要差异，必须人工核查。

验收结论更新为：M0 的固定传输延迟已通过上述连续输入与级联回归检查。
惯性延迟、脉冲过滤、模拟电压、物理时间校准与寄存器仍在范围之外。
